 ...
 function OpenSaveFileDialogMulti(ParentHandle: THandle; const DefExt, Filter, InitialDir,
   Title: string; var FileName: string; IsOpenDialog: Boolean): Boolean;
 ...
 function OpenSaveFileDialogMulti(ParentHandle: THandle; const DefExt, Filter, InitialDir, Title: string; var FileName: string; IsOpenDialog: Boolean): Boolean;
 var
   ofn: TOpenFileName;
   szFile: array[0..MAX_PATH] of Char;
 begin
   ...
   begin
     ...
     hwndOwner := ParentHandle;
     lpstrFile := szFile;
     Flags := OFN_HIDEREADONLY + OFN_AllowMultiSelect;
     nMaxFile := SizeOf(szFile);
     if (Title <> '') then
       lpstrTitle := PChar(Title);
     if (InitialDir <> '') then
